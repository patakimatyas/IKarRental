...
if($_GET){
    ...
    $id = $_GET['id'];
    $_SESSION['modifiableId'] = $id;
}else{
    $id = $_SESSION['modifiableId'];
}
...
$passengers = $_POST['passengers'] ?? $car['passengers'] ?? '';
$model = $_POST['model'] ?? $car['model'] ?? '';
$image = $_POST['image'] ?? $car['image'] ?? '';
$price = $_POST['price'] ?? $car['daily_price_huf'] ?? '';
$fuel = $_POST['fuel'] ?? $car['fuel_type'] ?? '';
$transmission = $_POST['transmission'] ?? $car['transmission'] ?? '';

function updateCars($carStorage, $id){
    global $year, $brand, $passengers, $model, $image, $price, $fuel, $transmission;
...
<head>
    ...
    <title>IKarRental - Autó módosítása</title>
...
<nav class="navbar bg-body-tertiary fixed-top">
    ...
    <div class="container mt-5" style="max-width: 600px;">
        ...
        <form action="modifyCar.php" method="post" enctype="multipart/form-data">
            <div class="mb-3">
